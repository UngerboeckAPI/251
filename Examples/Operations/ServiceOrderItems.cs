...
namespace Examples.Operations
{
  public class ServiceOrderItems : Base
  {
    ...
    /// <returns></returns>
    public ServiceOrderItemsModel Edit(string orgCode, int orderNumber, int orderLineNumber, string itemStatus, string userText01Val)
    {
      ...
      myServiceOrderItem.ServiceOrderItemUserFieldSets[0].UserText01 = userText01Val;
    ...
    public ServiceOrderItemsModel AddWithUserFields(string orgCode, EventsModel myEvent, int orderNumber, int orderLineNumber, string priceList, int priceListSeqNumber, int units, string issueClass, string issueType, string userText01Value)
    {
      var myServiceOrderItem = new ServiceOrderItemsModel
      {
        OrganizationCode = orgCode,
        ...
        PriceList = priceList,
        PriceListDetailSeqNbr = priceListSeqNumber,
        Units = units,
      ...
      myServiceOrderItem.ServiceOrderItemUserFieldSets = new List<UserFields>();
      var myUserField = new UserFields
      {
        Type = issueType, //This matches the value stored in Ungerboeck table column ER125_ISSUE_TYPE.
        Class = issueClass, //This matches the value stored in Ungerboeck table column ER125_ISSUE_CLASS.
        UserText01 = userText01Value //Set the value in the user field property
      };
      myServiceOrderItem.ServiceOrderItemUserFieldSets.Add(myUserField); //Then add it back into the RegistrationOrdersModel object.  You can add multiple user field sets to the same registration order object before saving.
